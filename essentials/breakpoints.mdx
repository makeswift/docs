...
<Warning>
  Custom breakpoints are only available with the new API pattern. Note that you
  need to pass the `runtime` instance to the `Makeswift` component and
  `MakeswiftPage` component.
</Warning>
