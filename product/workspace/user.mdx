...
<Frame>
  <img
    src="/images/makeswift-settings-users-send-invite.png"
    alt="An email address entered in the Send Invite field. The role dropdown selector is expanded showing roles for Guest, Editor, Publisher, and Author."
  />
</Frame>

Invited users will receive an email with a link to accept the invite. They'll be
redirected to Makeswift to finalize the process. If it's their first time using Makeswift,
they'll need to create an account after clicking the invite link. Existing users
must log in with the matching email address.
...
<Frame>
  <img
    src="/images/makeswift-settings-remove-user.png"
    alt="A popup window confirming deletion of a user. The dropdown selector next to the user is expanded showing the option for Remove user."
  />
</Frame>

## Permissions & Roles

There are five user roles, each with different permissions:
...
Guests have read only access to your workspace. They can utilize preview links to see changes made to the site, but are unable to move any elements on the canvas or change any additional settings or properties.

This permission level is best used for stakeholders that do not manage the site on a regular basis.
