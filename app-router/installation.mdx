...
The fastest way to get started with Makeswift for a new Next.js project is to follow the
...
```
  ...
  <Step title="Add API key to environment variables">
    ...
    Requesting data through the `Makeswift` client requires a site API key from Makeswift. First, open your `.env.local` file and paste the snippet below along with your other [environment variables](https://nextjs.org/docs/app/building-your-application/configuring/environment-variables).

    ```sh
    MAKESWIFT_SITE_API_KEY=paste-your-api-key-here
    ```

    Then, in the Makeswift builder, go to **Settings > Host** and copy the API key for the site and replace the value of the environment variable you pasted above.
...
Finally, navigate back to **Settings > Host**, and add your app's URL. If you haven't changed anything in the example and the server is running on port `3000`, the app's URL should be
`http://localhost:3000`.
...
<Info>
  When you're ready to deploy, set up a separate site and use your deployment
  URL instead of `http://localhost:3000`. You can keep this site for local
  development.
</Info>
