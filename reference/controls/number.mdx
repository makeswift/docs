...
<ParamField query="defaultValue" type="number">
  Defines the value shown for the number in the Makeswift builder when the value
  isn’t set. If you don’t provide a default value, the Checkbox control passes
  an `undefined` value to your component.
</ParamField>
...
<ParamField query="step" type="number" default={1}>
  Amount by which to increment the number value when using the ↑ ↓ arrows to
  drag the panel input.
</ParamField>
