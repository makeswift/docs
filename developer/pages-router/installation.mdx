...
Next.js plugins are configured in the project's next.config.js file by wrapping `nextConfig`. The Makeswift Next.js plugin whitelists Makeswift image domains.

<NextConfigExample />

  </Step>

<Step title="Add the Makeswift middleware">

Create a `middleware.ts` file at the same level as your `app` directory. The
`withMakeswiftMiddleware` adds middleware to your project to shows you the preview
version of your pages while in the builder. 

If you already have existing middleware in your Next.js project, see our
[Middleware guide](/developer/guides/middleware) for composing middleware.
