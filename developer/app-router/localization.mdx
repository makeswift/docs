...
<Note>
  This guide required version `0.24` of the Makeswift runtime. Refer to our
  [Upgrading to 0.24.0](/developer/upgrading/0.24.0) guide for details on how to
  update.
</Note>
...
<Step title="Add locales in the Makeswift Visual Builder">
...
To add a new locale, click the **+ Add locale** button. In this case, we've added **es-ES**.
...
You can modify and delete (except the default locale) a locale by hovering over the locale.

</Step>

<Step title="Configure next-intl for your application">
...
<Step title="Fetch snapshots by locale">
...
The first time you switch to a new locale (in this case switching from `en-US` to `es-ES`), you'll notice a message that informs you that the localized page inherits from the default locale. To stop inheriting from the default locale and start customizing the content for the localized page, click the **Edit for this locale** button. Note that you will continue to see the message about inheriting from the base locale until you click that button.
...
From now on, content changes are only associated with this locale and do not affect the base locale.
...
but nested on the locale's path. For example, `es` pages will be located on `example.com/es/`.

To use domain-based localization, you can add the domain to the locale in its settings. You'll also need to follow the `next-intl` documentation on [routing domains](https://next-intl.dev/docs/routing#domains) to configure the routing config to support this.
