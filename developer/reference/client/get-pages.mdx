...
<ParamField query="sortBy" type="string" default="path">
  The field to sort the result pages by. Can be any one of the following:
  `'title'`, `'path'`, `'description'`, `'createdAt'`, `'updatedAt'`.
</ParamField>

<ParamField query="sortDirection" type="string" default="asc">
  The direction of how the result pages are sorted. Can be either `'asc'` or
  `'desc'`.
</ParamField>
