...
<ParamField query="metadata" type="boolean | PageMetadataSettings" default="true">
  Controls which page metadata from Makeswift is rendered in the `<head>` of the page. More on this in the [Rendering metadata](#rendering-metadata) section below.
</ParamField>
