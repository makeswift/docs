...
Notice we are defining a `defaultValue` here. Because of this, the `heading` prop in our `Card` component is marked as required. If we didn't add the `defaultValue`, the prop would have to be marked as optional.
