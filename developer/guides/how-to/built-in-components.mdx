...
description: "Learn how to create built-in components that integrate into Makeswift."
---

import ProjectSetup from "/snippets/guides/project-setup.mdx";

Built-in components are React components that are registered and integrated into Makeswift. These typically become available to users in the Visual Builder to use while building layouts and pages. Let's see how to create and register a built-in component.
...
In this guide, we're going to build a `Card` component. This component will need a few different properties:

- `image` - the URL of the image
- `altText` - the alt text to use for the image
...
Here's an example of how those properties will show up in the Visual Builder for our `Card` component when we're finished.
...
Start by creating a `src/components/Card` directory with an `index.tsx` file inside of it. Inside of this file, we can start by defining a type for the component props based on our requirements.
...
Next, we cam define the component itself. In this example, we'll be using the [Link](https://nextjs.org/docs/pages/api-reference/components/link) and [Image](https://nextjs.org/docs/app/api-reference/components/image) components from Next.js. Ignoring styles, the final component will look like this.
...
export function Card({
  ...
  return (
    <div className={className}>
      {image && <Image src={image} alt={altText} objectFit="cover" fill />}
      <div>
        <h3>{heading}</h3>
        <p>{description}</p>
...
After creating the base React component, we need to register that component with Makeswift. This makes the component available in the Component Tray for users to drag onto the canvas.
...
<Warning>
  The `type` property underneath the hood is stored in the Makeswift database as
  a unique identifier. This means that changing the `type` property might cause
  side effects. For example, if you already have an instance of this component
  to the Canvas and then change the `type`, Makeswift won't recognize the
  original component. In this case, you'll see a "Component not found" message.
</Warning>
...
Additionally, you can include an optional `icon` property in your configuration object to specify which icon to be displayed along side your component.

See the [Component icons](/developer/reference/component-icons) reference page for a full list of available icons. In this example, we'll use the `image` icon.
...
Additionally, you can manually select which properties you want to be visually editable by passing an object with a `properties` like so:
...
Each Makeswift control has its own set of configuration options, so you'll need to check the documentation for full details on each one. We'll keep the defaults in this example, but for full details, refer to the docs for the [Style control params](/developer/reference/controls/style#params).
...
By default, the [Image](/developer/reference/controls/image) control will pass the URL of the image as a `string` to the React component which is what we want. However, it can also return an object that includes width and height properties like so:
...
For now, though, we'll just take the default output of a string.
...
Notice we are defining a `defaultValue` here. Beacause of this, the `heading` prop in our `Card` component is marked as required. If we didn't add the `defaultValue` the prop would have to be marked as optional.

Let's continue by adding two more `TextInput` controls for `altText` and `linkText`. Again, we'll set `label` and `defaultValue` for both.
...
- the dynamic API router in `src/app/api/makeswift/[...makeswift].ts`

If you've followed the [Installation](/developer/app-router/installation.mdx) guide for initial setup, you should already have a `src/makeswift/components.ts` file which is already imported into each of these three places. In this case, you can import your registration file here, and you'll be good to go.
...
To be able to see your component withing the Visual Builder, you'll need your Makeswift site to be connected to your locally running application. Again, follow the [Installation](/developer/app-router/installation.mdx) guide for details on how to do that if you aren't already set up.
...
<Frame>
  <img
    src="/images/developer/guides/how-to/built-in-components/card-component-tray-and-canvas.jpeg"
    alt="Properties of the card component"
  />
</Frame>
