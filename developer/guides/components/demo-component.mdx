...
![Final accordion component](/images/developer/guides/components/demo-component/demo-component/accordion-component.png)
...
For components to become available to use inside of Makeswift, they need to be registered. We’ll do this in the `Accordions.makeswift.tsx` file.
...
```tsx src/components/Accordions/Accordion.makeswift.tsx
...
import "src/components/Accordions/Accordions.makeswift";
```

Now, in Makeswift, we should see the `Accordions` component in the Component Toolbar, which we can then drag into the Canvas.

Once we drag the component onto the Canvas, notice the `label` we defined in the `Accordions.makeswift.tsx` appears in the Properties Sidebar. Later, when we make the component editable in Makeswift, properties that can be edited will appear here.
...
In the `index.tsx` file, update your `Accordions` component to use the radix UI React accordion component. The example uses filler content for each accordion item. Each item has a title that triggers the expansion of its associated content.

```tsx src/components/Accordions/index.tsx {1, 5-11, 12-19}
...
Restart your development server. In the Visual Builder, when you click on the Accordion title, notice how the accordion doesn’t expand. That’s because we’re in Build mode by default. To toggle the accordion, we need to switch from Build mode to Interact mode in the Component Toolbar.
...
```tsx src/components/Accordions/index.tsx {5-14, 18-21}
import * as Accordion from "@radix-ui/react-accordion";
...
We can access the Makeswift controls and have them return input from the user in Makeswift when we register the component in the `Accordions.makeswift.ts` file. Each control adds properties to the component that we can edit in Makeswift.

We want the user to be able to edit the title for an accordion item. For this, we can use the [`TextInput`](/developer/reference/controls/text-input) control. We'll import this into the `Accordions.makeswift.ts` file and add it to our `props` object.

```tsx src/components/Accordions/Accordions.makeswift.tsx {1, 11-14}
...
We also want the user to be able to add multiple accordion items, not just one. To support this, we can use the [`List`](/developer/reference/controls/list) control and put the `title` prop inside of it.

```tsx src/components/Accordions/Accordions.makeswift.tsx {1, 11-17}
...
```tsx src/components/Accordions/Accordions.makeswift.tsx {3, 13-22, 24}
import { runtime } from "src/makeswift/runtime";
...
Next, adjust the `index.tsx` file to accept the `accordions` prop.

```tsx src/components/Accordions/index.tsx {3-5, 7-9, 11, 13-16}
import * as Accordion from "@radix-ui/react-accordion";
...
export function Accordions({ accordions }: Props) {
...
Now, we can input as many accordion items as we want in Makeswift!
...
```tsx src/components/Accordions/Accordions.makeswift.tsx {3, 21}
import { runtime } from "src/makeswift/runtime";
...
```tsx src/components/Accordions/index.tsx {7, 33}
...
First, adjust the `Accordions.makeswift.ts` file to include a prop that accepts the `Style` control. In this case, we’ll call the prop `className`.

```tsx src/components/Accordions/Accordion.makeswift.tsx {3, 11}
import { runtime } from "src/makeswift/runtime";
...
```tsx src/components/Accordions/index.tsx {15, 23}
import { ReactNode } from "react";
...
```tsx src/components/Accordions/index.tsx {25-27, 32}
import { ReactNode } from "react";
