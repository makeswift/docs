...
[getting started guide](/getting-started/installation). If you have an existing Next.js application
...
Create a file for the Makeswift [runtime](/concepts/runtime) named `lib/makeswift/runtime.js`.
...
Create a file for the Makeswift [client](/concepts/client) named `lib/makeswift/client.js`
...
If you already have a `_document.js`, you can update it to [extend the Document](/guides/advanced-setup#extending-the-custom-document) from `@makeswift/runtime/next`.

# <a name="pages-route" />Add a route for Makeswift pages

Create an [optional catch-all route](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes#optional-catch-all-segments) named `[[...path]].js`. You will use the `Makeswift` client to fetch page [snapshots](/concepts/snapshots) and pass it to the `MakeswiftPage` component to render.
...
instead of `localhost`. You can keep this site for local development. Read more about [deploying to production](/guides/deploying).
